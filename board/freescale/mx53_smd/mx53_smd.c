...
#endif
#define MX53_ZEDI_VERSION1 1 //xnn
...
static struct fb_videomode lvds_xga ={
 ...
 "CLAA-WVGA", 60, 800, 480, 33300, 45, 211, 22, 23, 1, 1,
...
#endif
//  add by panzidon  for 3V3_EN pull up
void PULL_3v3_en_pull_up()
{
       // add by panzidong 
        // PATA_DATA5->GPIO_2_5  output mode, set value=1;
         u32 reg;
...
//  add by panzidon  for power  left led
void power_left_led()
{
       // add by panzidong 
        // PATA_DATA5->GPIO_2_5  output mode, set value=1;
         u32 reg;
        mxc_request_iomux(MX53_PIN_ATA_DATA5, IOMUX_CONFIG_ALT1);
...
//  end by panzidong for power left led


// add by panzidong for DC_PLUG_OUT
void dc_plug_out()
{
        u32 reg;
	mxc_request_iomux(MX53_PIN_NANDF_CS0,IOMUX_CONFIG_ALT1);
	// DC_PLUG_OUT -> NANDF_CS0 -> GPIO_6_11  output mode, set value = 1;
...
// end by panzidong for DC_PLUG_OUT
...
{       
        unsigned int reg;
	reg  =  readl(ESDCTL_BASE_ADDR);
	reg &=(~(1<<30));
	writel(reg,ESDCTL_BASE_ADDR);
...
//add by ljs for dc2 =1.25v
#define WM8325_DC2_ON_CONFIG (0x405d)
//end by ljs
/* xnn change ddr voltage from 1.55 to 1.5*/
#define WM8325_DC3_ON_CONFIG (0x4062)
#define WM8325_DC4_ON_CONFIG (0x4066)
#define WM8325_DC3DC4_VSEL_MASK_LOW (0x80)

#define WM8325_LDO4_ON_CONFIG (0x4072)
#define WM8325_LDO_VSEL_MASK_LOW (0x80)

#define WM8325_LDO_ENABLE (0x4051)
#define WM8325_LDO_STATUS (0x4053)

#define WM8325_PARENT_ID (0x4000)
#define WM8325_PARENT_ID_VALUE_LOW (0x46)
#define WM8325_PARENT_ID_VALUE_HIGH (0x62)
...
void setup_pmic_voltages(void)
{
	...
	ires=i2c_read(0x34, WM8325_PARENT_ID, 2, value, 2);
	DBG("setup_pmic_voltages i2c_read ok parentid value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
	DBG("\n");
...
printf("song\n");
	//ljsong ldo10 2.4v
		/*ldo10_out voltage=2.4v*/
		ires= i2c_read(0x34, 0x4084, 2, value, 2);
		printf("setup_pmic_voltages i2c_read ok ldo10[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);

		value[0] &= ~0x1f;
		value[0] |= 0x14;

		i2c_write(0x34, 0x4084, 2, value, 2);
		printf("setup_pmic_voltages i2c_write ok ldo10 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);

		ires= i2c_read(0x34, 0x4084, 2, value, 2);
		printf("setup_pmic_voltages i2c_read ok ldo10 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
		printf("\n");
...
//dc4
/* decrease DDR from 1.55 to 1.5 */
   ires= i2c_read(0x34, WM8325_DC4_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_read ok dc4 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
    value[1] &=WM8325_DC3DC4_VSEL_MASK_LOW;
    value[1] |= 0x1A; //0x85V+.025*(0x1A)=1.5V
    ires=i2c_write(0x34, WM8325_DC4_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_write ok dc4 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
...
//dc3
    ...
    ires=i2c_read(0x34, WM8325_DC3_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_read ok dc3 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
    value[1] &=WM8325_DC3DC4_VSEL_MASK_LOW;
    value[1] |= 0x1A; //0x85V+.025*(0x1A)=1.5V
    ires=i2c_write(0x34, WM8325_DC3_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_write ok dc3 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
...
//dc2
    ires=i2c_read(0x34, WM8325_DC2_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_read ok dc2 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
    value[1] &=WM8325_DC3DC4_VSEL_MASK_LOW;
    value[1] |= 0x3c; //0x85V+.025*(0x1A)=1.5V
    ires=i2c_write(0x34, WM8325_DC2_ON_CONFIG, 2, value, 2);
    DBG("setup_pmic_voltages i2c_write ok dc2 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
	...
	/*vldo4_out voltage=1.8v*/
	ires= i2c_read(0x34, WM8325_LDO4_ON_CONFIG, 2, value, 2);
	DBG("setup_pmic_voltages i2c_read ok ldo4 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
	value[1] &=WM8325_LDO_VSEL_MASK_LOW;
	value[1] |= 0x10; //=1.8V
	i2c_write(0x34, WM8325_LDO4_ON_CONFIG, 2, value, 2);
	DBG("setup_pmic_voltages i2c_write ok ldo4 value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
...
/*vldo4_out enable ldo4*/
	i2c_read(0x34, WM8325_LDO_STATUS, 2, value, 2);
	DBG("setup_pmic_voltages i2c_read ok ldo status value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
	if((value[1] & 0x8) == 0x0)
	{
		value[1] |= 0x8;
		i2c_write(0x34, WM8325_LDO_ENABLE, 2, value, 2);
			DBG("setup_pmic_voltages i2c_read ok ldo enable value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);

	}else{
			DBG("setup_pmic_voltages i2c_read ok ldo already open value[1]=%x,value[0]=%x,ires=%d\r\n",value[1],value[0],ires);
...
int esdhc_gpio_init(bd_t *bis)
{
	...
	for (index = 0; index < CONFIG_SYS_FSL_ESDHC_NUM;
		...
		//	mxc_iomux_set_pad(MX53_PIN_ATA_IORDY, 0xD4);
			mxc_iomux_set_pad(MX53_PIN_ATA_IORDY, 0xC4);
...
//add by ljsong,or  will flash the screen
#if 0
...
//add by zed
	//cp bmp in sd or inand to fb derectly
	char s[50]={0};
	ulong addr=0x77afffd0;

	if(get_boot_device()==SD_BOOT)
		sprintf(s,"mmc dev 0");
	if(get_boot_device()==MMC_BOOT)
		sprintf(s,"mmc dev 1");
	run_command(s,0);


	sprintf(s,"mmc read 0x%08x 0x4000 0x900",addr);
	run_command(s,0);
return ;
...
int board_late_init(void)
{
	...
	#ifdef MX53_ZEDI_VERSION1 //xnn
		//xnn debug
		printf("xnn change wm8325 to decrease DDR voltage from 1.55 to 1.5,open ldo4\r\n");
	#endif


    //xnn change to wm8325 driver, decrease DDR voltage from 1.55 to 1.5
	setup_pmic_voltages();
...
#ifdef MX53_ZEDI_VERSION1 //xnn
    //xnn, pre setting gpio
    //wifi status led, EIM_D22, GPIO3_22, default pull down
		 printf("xnn:wifi status led, EIM_D22, GPIO3_22, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_EIM_D22,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_EIM_D22,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //GPS status led, PATA_DIOR, GPIO7_3, default pull down
printf("xnn:GPS status led, PATA_DIOR, GPIO7_3, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_ATA_DIOR,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_ATA_DIOR,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //GSM mobile  status led, EIM_CS1, GPIO2_24, default pull down
printf("xnn:GSM mobile  status led, EIM_CS1, GPIO2_24, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_EIM_CS1,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_EIM_CS1,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //TD-SCDMA status led, EIM_RW, GPIO2_26, default pull down
printf("xnn:TD-SCDMA status led, EIM_RW, GPIO2_26, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_EIM_RW,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_EIM_RW,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //GSM unicom status led, KEY_ROW2, GPIO4_11, default pull down
printf("xnn:GSM unicom status led, KEY_ROW2, GPIO4_11, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_KEY_ROW2,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_KEY_ROW2,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //WCDMA unicom status led, KEY_ROW4, GPIO4_15, default pull down
printf("xnn:WCDMA unicom status led, KEY_ROW4, GPIO4_15, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_KEY_ROW4,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_KEY_ROW4,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //CDMA/EVDO telecom status led, EIM_EB3, GPIO2_31, default pull down
printf("xnn:CDMA/EVDO telecom status led, EIM_EB3, GPIO2_31, default pull down\r\n");
        mxc_request_iomux(MX53_PIN_EIM_EB3,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_EIM_EB3,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //system status led, EIM_D20, GPIO3_20, default pull down
printf("xnn:system status led, EIM_D20, GPIO3_20, default pull down\r\n");
            mxc_request_iomux(MX53_PIN_EIM_D20,IOMUX_CONFIG_ALT1);
        mxc_iomux_set_pad(MX53_PIN_EIM_D20,
			  PAD_CTL_SRE_FAST |
			  PAD_CTL_ODE_OPENDRAIN_ENABLE |
			  PAD_CTL_DRV_HIGH | PAD_CTL_100K_PU |
			  PAD_CTL_HYS_ENABLE);
    ...
    //system warning led, EIM_D21, GPIO3_21, default pull up to down
printf("xnn:system warning led, EIM_D21, GPIO3_21, default pull up to down\r\n");
        mxc_request_iomux(MX53_PIN_EIM_D21,IOMUX_CONFIG_ALT1);
...
/**
 * 配置为lcd模式 add by ljsong
 * 
 */
void imx53_lcd_config(void)
{
...
#if 0
//default
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT0, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT1, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT2, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT3, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT4, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT5, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT6, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT7, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT8, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT9, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT10, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT11, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT12, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT13, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT14, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT15, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT16, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT17, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT18, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT19, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT20, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT21, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT22, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT23, PAD_CTL_DRV_LOW);

        mxc_iomux_set_pad(MX53_PIN_DI0_DISP_CLK, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN2, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN3, PAD_CTL_DRV_LOW);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN15, PAD_CTL_DRV_LOW);
...
#if 1
        //ok
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT0, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT1, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT2, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT3, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT4, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT5, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT6, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT7, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT8, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT9, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT10, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT11, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT12, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT13, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT14, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT15, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT16, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT17, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT18, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT19, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT20, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT21, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT22, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT23, PAD_CTL_DRV_HIGH);

        mxc_iomux_set_pad(MX53_PIN_DI0_DISP_CLK, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN2, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN3, PAD_CTL_DRV_HIGH);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN15, PAD_CTL_DRV_HIGH);
#endif



#if 0
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT0, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT1, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT2, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT3, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT4, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT5, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT6, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT7, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT8, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT9, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT10, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT11, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT12, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT13, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT14, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT15, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT16, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT17, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT18, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT19, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT20, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT21, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT22, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DISP0_DAT23, PAD_CTL_DRV_MEDIUM);

        mxc_iomux_set_pad(MX53_PIN_DI0_DISP_CLK, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN2, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN3, PAD_CTL_DRV_MEDIUM);
        mxc_iomux_set_pad(MX53_PIN_DI0_PIN15, PAD_CTL_DRV_MEDIUM);
#endif
 ...
 * add by zed
 */
void imx53_5v_on(void){

   u32 reg;
   mxc_request_iomux(MX53_PIN_EIM_OE, IOMUX_CONFIG_ALT1);
   reg = readl(GPIO2_BASE_ADDR + GPIO_GDIR);
   reg |= (1<<25);
   writel(reg, GPIO2_BASE_ADDR + GPIO_GDIR);
 ...
 */
#define DBG(...) printf("%s,%d : ",__FUNCTION__,__LINE__);printf(__VA_ARGS__)
int imx53_check_recovery_key(void){

#if 1
			u32 reg;
	//set c4 out
			mxc_request_iomux(MX53_PIN_KEY_COL4, IOMUX_CONFIG_ALT1);
			reg = readl(GPIO4_BASE_ADDR + GPIO_GDIR);
			reg |= (1<<14);
		    writel(reg, GPIO4_BASE_ADDR + GPIO_GDIR);
...
#ifdef CONFIG_VIDEO_MX5
void imx53_pwm0_enalbe()
{
	imx_pwm_enable(pwm0);
}

void imx53_pwm0_disalbe()
{
	imx_pwm_disable(pwm0);
}


void imx53_pwm0_setval(int val)
{
	if(val>50000)
		val=50000;
	if(val<0)
		val=0;
	imx_pwm_config(pwm0, val, 50000);
}
#endif
...
void update_logo(void){
	...
	do{
		...
		if(get_boot_device()==MMC_BOOT){
			...
			//switch sd
			sprintf(s,"mmc dev 0");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;

			sprintf(s,"fatload mmc 0 0x97afffd0 image/logo.bmp");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;
			...
			sprintf(s,"mmc write 0x97afffd0 0x4000 0x900");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;

			printf("update logo success\n");
	...
	else if(get_boot_device()==SD_BOOT)
	{
		//switch sd
...
void update_uboot(void){
	...
	do{
		...
		if(get_boot_device()==MMC_BOOT){
			...
			sprintf(s,"fatload mmc 0 0x70800000 image/u-boot.bin");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;
			...
			sprintf(s,"mmc write 0x70800400 2 0x500");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;

			printf("update u-boot success\n");
...
void update_uImage(void){
	...
	do{
		...
		if(get_boot_device()==MMC_BOOT){
			...
			sprintf(s,"fatload mmc 0 0x70800000 image/uImage");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;
			...
			sprintf(s,"mmc write 0x70800000 0x800 0x2200");
			iRes=run_command(s,0);
			if(iRes==-1)
				break ;

			printf("update uImage success\n");
