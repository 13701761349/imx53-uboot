...
extern int imx53_check_recovery_key(void);
int check_key_pressing(void)
{
	return imx53_check_recovery_key();
}
#endif
...
//add by ljs for update
void setup_recovery_env(const char *sValue)
{

	printf("-----------------------%d,%s,sValue=%s\n",__LINE__,__FUNCTION__,sValue);

	char *env, *boot_args, *boot_cmd;
	int bootdev = get_boot_device();

	printf("-----bootdev = %d\n",bootdev);
	//only boot from inand then can go to update
	if(MMC_BOOT!=bootdev)
		return ;
...
//	setup_recovery_env("1");

#if 0
	if (check_key_pressing())
		setup_recovery_env("1");
	else
		setup_recovery_env("0");
#endif
