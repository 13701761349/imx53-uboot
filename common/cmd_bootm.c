...
void re_ext2load(void)
{

	char *p_bootcmd=getenv("bootcmd");
	printf("load uImage failed , then to load uImage.bak\n");
	printf("bootcmd=%s\n",p_bootcmd);
	if(strstr(p_bootcmd,"sd")!=NULL)
	{

		char *p_bootcmd_sd=getenv("bootcmd_sd");
		printf("bootcmd_sd=%s\n",p_bootcmd_sd);
		char p_bootcmd_sd_new[256]={0};	
		strcpy(p_bootcmd_sd_new,p_bootcmd_sd);
		
		ReplaceStr(p_bootcmd_sd_new, "uImage", "abcdefghij");
		ReplaceStr(p_bootcmd_sd_new, "abcdefghij", "uImage.bak");
		printf("bootcmd_sd_new=%s\n",p_bootcmd_sd_new);
		
		run_command(p_bootcmd_sd_new,0);
		return 0;

		
	}else if(strstr(p_bootcmd,"mmc")!=NULL)
	{
		char *p_bootcmd_mmc=getenv("bootcmd_mmc");
		printf("bootcmd_mmc=%s\n",p_bootcmd_mmc);
		char p_bootcmd_mmc_new[256]={0};	
		strcpy(p_bootcmd_mmc_new,p_bootcmd_mmc);
		
		ReplaceStr(p_bootcmd_mmc_new, "uImage", "abcdefghij");
		ReplaceStr(p_bootcmd_mmc_new, "abcdefghij", "uImage.bak");
		printf("p_bootcmd_mmc_new=%s\n",p_bootcmd_mmc_new);
		
		run_command(p_bootcmd_mmc_new,0);
		return 0;
...
static int bootm_start(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (images.os.image_len == 0) {
		...
		goto next_fail;//for suzhou youke load uImage.bak from ext2 file system
		return 1;
	}
...
static int bootm_load_os(image_info_t os, ulong *load_end, int boot_progress)
{
	...
	printf("load=%x,blob_start(os.start)=%x\n",load,blob_start);
	printf("load=%x,image_start(os.image_start)=%x\n",load,image_start);
	switch (comp) {
	case IH_COMP_NONE:
		if (load == blob_start) {
...
{
// add by zed
extern   void imx53_licenses(void);
         //     imx53_licenses();
	...
	imx53_pwm0_disalbe();
	run_command("mw.b 0x97b00000 0 119436",0);
	//end add by ljs
#endif
